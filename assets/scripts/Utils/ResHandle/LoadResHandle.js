...
var LoadResHandle = cc.Class({
    ...
    statics: {
        ...
        init:function()
        {
            ...
            // console.log("LoadResHandle KeyList:"+this.KeyList.length);
            // for(var i = 0;i < this.KeyList.length;i++)
            // {
            //     console.log("LoadResHandle KeyList[]:"+this.KeyList[i]);
            // }
            // this.removeByValue(this.KeyList,"ss2"); 

            // console.log("LoadResHandle removeByValue KeyList:"+this.KeyList.length);
            // for(var i = 0;i < this.KeyList.length;i++)
            // {
            //     console.log("LoadResHandle removeByValue KeyList[]:"+this.KeyList[i]);
            // }
            ...
            // console.log("LoadResHandle AddToList WaitList:"+this.WaitList.length);
            // console.log("LoadResHandle AddToList delete ss length:"+Object.keys(this.WaitList).length);

        },
        testcallback:function(err,image)
        {
            console.log("LoadResHandle testcallback:"+image);
        },
       ...
       DownloadByKey:function(key,url,callback)
       {
            ...
            this.CheckUpdate();
            console.log("LoadResHandle AddToList:"+key);
            console.log("LoadResHandle AddToList length:"+Object.keys(this.WaitList).length);
            console.log("LoadResHandle AddToList end length:"+Object.keys(this.WaitList).length);
       },
       ...
       DownloadByUrl:function(url,callback)
       {
            ...
            this.ResIndex++;
            console.log("LoadResHandle DownloadByUrl:"+key);
            //数据过大，重置
       ...
       update:function()
       {
            cc.vv.LoadResHandle.IsUpdate = true;
            console.log("LoadResHandle update+");
            cc.vv.LoadResHandle.DownloadRes();
       },
       DownloadRes:function()
       {
            if(Object.keys(this.WaitList).length <= 0)
            {
                ...
                {
                    //关闭update
                    console.log("LoadResHandle update stop");
                    this.IsUpdate = false;
            ...
            else//数组中要下载的列表还有
            {
                ...
                if(this.CurDownloadKey == null || this.CurDownloadKey == "undefined")
                {
                    ...
                    console.log("LoadResHandle this.CurDownloadKey:"+this.CurDownloadKey);
                    console.log("LoadResHandle this.KeyList[0]:"+this.KeyList[0]);
                    var self = this;
                    console.log("LoadResHandle this.CurDownloadKey-url:"+this.WaitList[this.CurDownloadKey]);
                    cc.loader.load(this.WaitList[this.CurDownloadKey], function (err, tex)
                    {
                        if(err == null)
                        {
                            //下载完成，
                            //删除KeyList中的key
                            console.log("LoadResHandle 下载完成，:"+self.CurDownloadKey);
                            ...
                            {
                                console.log("LoadResHandle 下载完成，dlSpriteFrame:"+dlSpriteFrame);
                                self.spriteList[self.CurDownloadKey].SpriteFrame = dlSpriteFrame;
                            ...
                            {
                                console.log("LoadResHandle 下载完成，CurDownloadKey:"+err);
       ...
       {
            console.log("LoadResHandle CheckUpdate this.IsUpdate:"+this.IsUpdate);
            if(this.IsUpdate == false)
            {
                console.log("LoadResHandle this.IsUpdate = false:"+Object.keys(this.WaitList).length);
                if(Object.keys(this.WaitList).length > 0)
                {
                    console.log("LoadResHandle CheckUpdate IsUpdate = false :setInterval");
                    this.IntervalID = setInterval(this.update,1000);
                    this.IsUpdate = true;
                    
                    return;
                }
                console.log("LoadResHandle CheckUpdate IsUpdate = false :clearInterval");
                this.IsUpdate = false;
                clearInterval(this.IntervalID);
            }
            else if(this.IsUpdate == true)
            {
                console.log("LoadResHandle this.IsUpdate = true:"+Object.keys(this.WaitList).length);
                console.log("LoadResHandle this.IsUpdate = true this.CurDownloadKey:"+this.CurDownloadKey);
                if(Object.keys(this.WaitList).length <= 0 && (this.CurDownloadKey == null || this.CurDownloadKey == "undefined"))
                {
                    console.log("LoadResHandle CheckUpdate IsUpdate = ture:clearInterval");
                    this.IsUpdate = false;
                    clearInterval(this.IntervalID);


                    console.log("LoadResHandle CheckUpdate WaitList length:"+Object.keys(this.WaitList).length);
                    console.log("LoadResHandle CheckUpdate DoneList length:"+Object.keys(this.DoneList).length);
                    console.log("LoadResHandle CheckUpdate DoneList length:"+Object.keys(this.spriteList).length);
                    
                    console.log("LoadResHandle CheckUpdate KeyList length:"+this.KeyList.length);
                    console.log("LoadResHandle CheckUpdate CurDownloadKey :"+this.CurDownloadKey);
